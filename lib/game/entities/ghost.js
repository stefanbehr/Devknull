...
.defines(function() {
			...
			if (player) {
				...
				if (enemyDist <= this.attackRadius) {
					if (enemyDist > this.stopRadius) {
						this.activeWeapon = 'whirlwind';
						this.stopped = false;
						if (this.whirlwindInterval <= this.attackTimer.delta()) {
							this.attacking = true;
    ...
    EntityGhostWeapon = ig.Entity.extend({
		...
		setupAnimation: function(dir) {
            ...
            this.addAnim('whirlwind', 1, [16]);
        },
        checkAttack: function() {
            if (this.weaponType == 'whirlwind' && this.owner.attacking == true) {
                this.attacking = false;
                this.attackTimer = 0;
                this.hasHit = false;
                ig.game.spawnEntity('EntityWhirlwind', this.owner.pos.x, this.owner.pos.y, {dir:this.owner.direction, owner:this.owner});
            }
         },
    });
});
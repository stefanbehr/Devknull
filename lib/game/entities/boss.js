...
.defines(function() {
	EntityBoss = ig.Entity.extend({
		animSheet: new ig.AnimationSheet('media/boss.png', 42, 57.5),
		size: {x: 42, y: 57.5},
        offset: {x: 0, y: 0},
        name: 'boss',
        zIndex: -7,
		...
		init: function(x, y, settings) {
			...
			this.weapon = ig.game.spawnEntity('EntityBossWeapon', this.pos.x, this.pos.y, {owner:this});
			// Add animations
		...
		kill: function() {
			this.parent();
			var spawn = ig.game.getEntitiesByType('EntityPrincessSpawn')[0];
			ig.game.spawnEntity('EntityPrincess', spawn.pos.x, spawn.pos.y);
		},
		tryFollowingPlayer: function(player) {
			...
			if (player) {
				...
				if ((enemyDist <= this.followRadius) && (enemyDist > this.stopRadius)) {
					...
					yneighborhood = (Math.abs(this.pos.x - px) < 50);
					xneighborhood = (Math.abs(this.pos.y - py) < 50);
				...
				if (enemyDist <= this.attackRadius) {
					if (enemyDist > this.stopRadius) {
						...
						this.stopped = false;
						if (this.fireballInterval <= this.attackTimer.delta()) {
							this.attacking = true;
    ...
    EntityBossWeapon = ig.Entity.extend({
        size: {x: 32, y: 32}, //TODO ANIMATE WEAPON
        animSheet: new ig.AnimationSheet('media/empty.png', 32, 32),
        maxVel: {x: 150, y: 150},
        type: ig.Entity.TYPE.NONE,
        checkAgainst: ig.Entity.TYPE.A,
        collides: ig.Entity.COLLIDES.PASSIVE,
		...
		setupAnimation: function(dir) {
            ...
            this.addAnim('fireball', 1, [offset]);
            this.addAnim('swing', this.attackDuration, swingseq);
        },
        checkAttack: function() {
            if (this.weaponType == 'sword') {
                if ((this.owner.attacking ) && !this.attacking) {
                	console.log("owner attacking, this not");
                    this.attacking = true;
                    this.hasHit = false;
                    this.attackTimer = new ig.Timer();
                } else if (this.attacking) {
                	console.log("elif this attacking");
            ...
            if ((this.weaponType == 'fireball') && (this.owner.attacking )) {
                this.attacking = false;
                this.attackTimer = 0;
                this.hasHit = false;

...
.defines(function() {
	EntityPlayer = ig.Entity.extend({
		...
		update: function() {
            ...
            if (ig.input.pressed('switch')) {
                this.activeWeapon = (this.activeWeapon == this.weapons[0]) ? this.weapons[1] : this.weapons[0];
                this.weapon.kill();
                this.weapon = ig.game.spawnEntity('EntityWeapon', this.pos.x, this.pos.y, {owner:this});
            }
    ...
    EntityWeapon = ig.Entity.extend({
        size: {x: 1000, y:1000}, //TODO ANIMATE WEAPON
        animSheet: new ig.AnimationSheet('media/weapons1.png', 5, 3),
        ...
        checkAttack: function() {
            if (this.weaponType == 'sword') {
                if (ig.input.pressed('attack') && !attacking) {
                    this.attacking = true;
                    this.hasHit = false;
